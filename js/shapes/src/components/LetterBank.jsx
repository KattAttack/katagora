...
export class LetterBank extends Component {
	constructor(props) {
		...
		this.letterSelectionFunc = this.letterSelectionFunc.bind(this);
	...
	letterSelectionFunc(item) {
		...
		for (var i = 0; i < secretWord.length; i++) {
			...
			// console.log("ready for another check");
		}
		if (correctLetterClicked === false) {
			this.props.wrongGuess();
		}
		// console.log("Clicked Letters Array", this.state.clickedLettersArr);
	}
	...
	render() {
		...
		console.log("Correct Letters Clicked", correctClickedLetterCount);

		if (correctClickedLetterCount === secretWord.length || wrongLetterCount === svgObjLength) {
			...
			if (correctClickedLetterCount === secretWord.length) {
				return (
					<div>
		...
		var renderedOutput = letters.map(item => {
			...
			if (clickedLettersArr.length >= 1) {
				...
				return (
					<div className='LetterContainer' onClick={e => this.letterSelectionFunc(item)}>
						{" "}
						{item}{" "}
					</div>
				);
